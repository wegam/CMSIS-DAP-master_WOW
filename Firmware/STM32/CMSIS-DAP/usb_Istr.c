...
U32 USBD_ReadEP(U32 EPNum, U8 *pData)
{
	...
	pv  = (U32 *)(USB_PMA_ADDR + 2 * ((pBUF_DSCR + num)->ADDR_RX));		//取得些端点的缓冲区地址
...
U32 MSC_ReadEP(U32 EPNum, U8 *pData)
{
	...
	num = EPNum & 0x0F;
//	pv  = (U32 *)((pBUF_DSCR + num)->ADDR_RX);
//	pv  = (U32 *)(2*pv);
//	pv  = (U32 *)(pv+USB_PMA_ADDR);
	pv  = (U32 *)(USB_PMA_ADDR + 2 * ((pBUF_DSCR + num)->ADDR_RX));		//取得些端点的缓冲区地址
//	pv=(U32 *)0X400060A0;
	cnt = (pBUF_DSCR + num)->COUNT_RX & EP_COUNT_MASK;								//计算接收到的数据个数
	for (n = 0; n < (cnt + 1) / 2; n++)
...
void USB_Istr(void)
{
	...
	istr = ISTR;									//中断状态寄存器
	//1)复位中断请求--复位USB数据
	if (istr & ISTR_RESET)		//复位串口为默认状态9600
	{	/* USB Reset Request */
		USBD_Reset();						//复位
		usbd_reset_core();			//复位USB的一些数据标志
		if (USBD_P_Reset_Event)
	...
	//7)期望帧首标识位 (Expected start of frame)
	if (istr & ISTR_ESOF)					//期望帧首标识位 (Expected start of frame)
	{	
//		//此位在USB模块未收到期望的SOF分组时由硬件置位。主机应该每毫秒都发送SOF分组，
//		//但如果USB模块没有收到，挂起定时器将触发此中断。如果连续发生3次ESOF中断，
...
//		 #endif
	...
	//8)正确的传输 (Correct transfer)此位在端点正确完成一次数据传输后由硬件置位。
