...
/*******************************************************************************
*函数名			:	USBD_CDC_ACM_PortInitialize
*功能描述		:	串口配置/初始化
*输入				: 
*返回值			:	无
*******************************************************************************/
...
int32_t UART_WriteData (uint8_t *data, uint16_t size)
{
	...
	if (cnt != 0)
		USART_ITConfig(USART_PORT, USART_IT_TXE, ENABLE);				//开启中断，进入中断处理函数
...
void USART_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART_PORT, USART_IT_TXE) != RESET)
	{
		...
		{
			USART_SendData(USART_PORT, WrBuffer.data[WrBuffer.idx_out++]);					//串口发送数据
