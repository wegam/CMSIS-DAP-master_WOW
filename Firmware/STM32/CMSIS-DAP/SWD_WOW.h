...
#define DP_IDCODE      0x00        // ±êÊ¶´úÂë¼Ä´æÆ÷		IDCODE Register (Read only)
#define DP_ABORT       0x00        // ÖÐÖ¹¼Ä´æÆ÷				Abort Register (Write only)
#define DP_CTRL_STAT   0x04        // ¿ØÖÆ/×´Ì¬¼Ä´æÆ÷		Control & Status
#define DP_WCR         0x04        // Ïß¿ØÖÆ¼Ä´æÆ÷			Wire Control Register (SW Only)
#define DP_DLCR        0x04        // Data Link Control Register (Renamed WCR)
#define DP_SELECT      0x08        // APÑ¡Ôñ¼Ä´æÆ÷			Select Register (JTAG R/W & SW W)
#define DP_RESEND      0x08        // ¶ÁÔÙ·¢¼Ä´æÆ÷			Resend (SW Read Only)
#define DP_RDBUFF      0x0C        // ¶Á»º³å¼Ä´æÆ÷			Read Buffer (Read Only)
...
/* DAPÖÐÖ¹¼Ä´æÆ÷½á¹¹Ìå */
typedef __packed struct _DAP_WOW_ABORT
{
  uint32_t Request_Abort;
} DAP_WOW_ABORT;

void SWD_WOW_DELAY(uint32_t time);
void SWD_WOW_DISABLE(void);
void SWD_WOW_ENABLE(void);
void SWD_WOW_CLOCK_CYCLE(void);
