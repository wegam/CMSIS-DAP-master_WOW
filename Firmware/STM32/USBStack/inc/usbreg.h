...
#define USB_BASE_ADDR   0x40005C00  /* USB Registers Base Address */		//USB模块基地址--0x40005C00
#define USB_PMA_ADDR    0x40006000  /* USB Packet Memory Area Address */


/* Common Registers */
#define CNTR    REG(USB_BASE_ADDR + 0x40)   /* Control Register */							//USB控制寄存器
#define ISTR    REG(USB_BASE_ADDR + 0x44)   /* Interrupt Status Register */			//USB中断状态寄存器
#define FNR     REG(USB_BASE_ADDR + 0x48)   /* Frame Number Register */					//USB帧编号寄存器
#define DADDR   REG(USB_BASE_ADDR + 0x4C)   /* Device Address Register */				//USB设备地址寄存器
#define BTABLE  REG(USB_BASE_ADDR + 0x50)   /* Buffer Table Address Register */	//USB分组缓冲区描述表地址寄存器
