...
extern void  usbd_connect               (BOOL con);	//控制USB的连接与断开
extern void  usbd_reset_core            (void);			//复位USB的一些数据标志
extern BOOL  usbd_configured            (void);

/* USB Device user functions imported to USB HID Class module                 */
extern void  usbd_hid_init              (void);																////HID初始化--将参数设置为默认值
...
extern int32_t  USBD_CDC_ACM_PortInitialize            (void);				//串口配置/初始化
