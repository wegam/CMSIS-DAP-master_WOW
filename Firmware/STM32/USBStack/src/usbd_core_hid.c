...
/*******************************************************************************
*函数名			:	USBD_ReqGetDescriptor_HID
*功能描述		:	HID描述符，包括设备描述符，报告描述符，物理特性描述符
*输入				: 
*返回值			:	无
*******************************************************************************/
/*
...
__weak BOOL USBD_ReqGetDescriptor_HID (U8 **pD, U32 *len)			//HID描述符，包括设备描述符，报告描述符，物理特性描述符
{
  switch (USBD_SetupPacket.wValueH)
	{
    case HID_HID_DESCRIPTOR_TYPE:			//HID描述符类型
      ...
      *len = USB_HID_DESC_SIZE;
      break;
    case HID_REPORT_DESCRIPTOR_TYPE:	//HID报告描述符类型
      if (USBD_SetupPacket.wIndexL != usbd_hid_if_num)
			{
        return (__FALSE);  /* Only Single HID Interface is supported */
      }
      ...
      *len = USBD_HID_ReportDescriptorSize;
      break;
    case HID_PHYSICAL_DESCRIPTOR_TYPE:	//HID物理特性描述符类
