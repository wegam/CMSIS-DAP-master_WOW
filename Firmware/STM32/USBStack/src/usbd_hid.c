...
BOOL         DataOutAsyncReq;				/* Asynchronous data out request	*/	//异步数据输出请求
...
__weak void  usbd_hid_init         (void)                                        {};								//HID初始化--将参数设置为默认值
__weak int   usbd_hid_get_report   (U8  rtype, U8 rid, U8 *buf, U8  req)         { return (0); };		//usbd_user_hid.c
__weak void  usbd_hid_set_report   (U8  rtype, U8 rid, U8 *buf, int len, U8 req) {};								//usbd_user_hid.c
...
void USBD_HID_EP_INTOUT_Event (U32 event)
{
  ...
  if (!DataInReceLen) 	/* Check if new reception	*/		//（U16）DataInReceLen==0
	{                 
    ptrDataIn     = USBD_HID_OutReport;									//（U8*）ptrDataIn	,	USBD_HID_OutReport[USBD_HID_OUTREPORT_MAX_SZ+1];
    DataInReceLen = 0;
  }
  bytes_rece      = USBD_ReadEP(usbd_hid_ep_intout, ptrDataIn);			//读取端点3并响应主机，数据存入ptrDataIn，返回接收计数值
  ptrDataIn      += bytes_rece;			//地址增加bytes_rece（接收到个数）
  DataInReceLen  += bytes_rece;			//计数增加bytes_rece（接收到个数）
  if (!bytes_rece ||(DataInReceLen >= usbd_hid_outreport_max_sz)||(bytes_rece	<	usbd_hid_maxpacketsize[USBD_HighSpeed])) 			//usbd_hid_outreport_max_sz==64,
	{
    if (usbd_hid_outreport_num <= 1) 			//usbd_hid_outreport_num恒等于1
		{  /* If only one out report in system   */
      usbd_hid_set_report (HID_REPORT_OUTPUT,                    0 ,  USBD_HID_OutReport   , DataInReceLen,   USBD_HID_REQ_EP_INT);		//读取USB接收缓冲区数据并保存到USB_Request数组中
    }
		else 
		{
      usbd_hid_set_report (HID_REPORT_OUTPUT, USBD_HID_OutReport[0], &USBD_HID_OutReport[1], DataInReceLen-1, USBD_HID_REQ_EP_INT);		//读取USB接收缓冲区数据并保存到USB_Request数组中
    }
    DataInReceLen = 0;			//数据接收完成，清除计数
  }
}

/*******************************************************************************
*函数名			:	USBD_HID_Configure_Event
*功能描述		:	USB设备HID配置回调
*输入				: 
*返回值			:	无
*******************************************************************************/
...
void USBD_HID_EP_INT_Event (U32 event) {
  if (event & USBD_EVT_IN) {				//设备输出--电脑输入
    USBD_HID_EP_INTIN_Event  (event);
  }
  if (event & USBD_EVT_OUT) {				//OUT Packet电脑输出包
    USBD_HID_EP_INTOUT_Event (event);
  }
}

/*******************************************************************************
*函数名			:	USBD_HID_SOF_Event
*功能描述		:	HID帧首中断标志，帧开始中断
*输入				: 
*返回值			:	无
*******************************************************************************/
/*
...
void USBD_HID_SOF_Event (void)
{
  ...
  if (USBD_Configuration)			//USBD_Configuration==0,未配置，USBD_Configuration!=0,已配置
	{
		tick_4ms = __FALSE;				//tick_4ms =0;
		if (cnt_for_4ms++ >= ((4 << (3 * USBD_HighSpeed))) - 1)
		{
			cnt_for_4ms = 0;
			tick_4ms    = __TRUE;		//tick_4ms =1;
		}

    polling_reload = __FALSE;	//polling_reload = 0;
    ...
    for (i = 0; i < usbd_hid_inreport_num; i++)
      ...
      if (polling_reload)
			{
        if (do_polling)		/* If polling is longer than idle     */
				{
          DataOutUpdateReqMask |= (1 << i);
        }
      }
    ...
    if (DataOutUpdateReqMask && !DataOutToSendLen)
		{        /* If pending     */
...
/*******************************************************************************
*函数名			:	usbd_hid_get_report_trigger
*功能描述		:	USB设备HID获取报告触发器//将上报数据传入USB响应缓冲器
*输入				: 
*返回值			:	无
*******************************************************************************/
/*
...
{
  if (len > usbd_hid_inreport_max_sz)		//usbd_hid_inreport_max_sz==64	//传入数据过长，返回错误
    return (__FALSE);

  if (USBD_Configuration)			//USBD_Configuration==0,未配置，USBD_Configuration!=0,已配置
	{
    DataOutAsyncReq    = __TRUE;        /* Asynchronous data out request	*/	//异步数据输出请求
    while (DataOutToSendLen)
		{
      if (!USBD_Configuration)
			{        /* If device not configured reject rq */
        DataOutAsyncReq    = __FALSE;   /* Asynchronous data out request	*/	//异步数据输出请求
        ptrDataOut         = NULL;
        DataOutSentLen     = 0;
        DataOutToSendLen   = 0;
        return (__FALSE);
    ...
    memcpy (&USBD_HID_InReport[1], buf, len);		//拷贝数据，
    ptrDataOut             = USBD_HID_InReport;
    DataOutSentLen         = 0;
