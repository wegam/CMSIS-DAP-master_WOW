...
int8_t  data_read_access;              /*!< Flag active while read data (in the receive intermediate buffer) is being accessed */
...
int8_t  data_receive_int_access;       /*!< Flag active while read data (in the receive intermediate buffer) is being accessed from the IRQ function*/
int32_t  data_received_pending_pckts;   /*!< Number of packets received but not handled (pending) */	//已接收的未处理的数据包个数
...
/* Functions that should be provided by user to use standard Virtual COM port
   functionality                                                              */
__weak int32_t USBD_CDC_ACM_PortInitialize          (void)                             { return (0); };		//串口配置/初始化
__weak int32_t USBD_CDC_ACM_PortUninitialize        (void)                             { return (0); };
__weak int32_t USBD_CDC_ACM_PortReset               (void)                             { return (0); };
__weak int32_t USBD_CDC_ACM_PortSetLineCoding       (CDC_LINE_CODING *line_coding)     { return (0); };		//设置串口参数 //usbd_user_cdc_acm.c line 121
...
__weak int32_t USBD_CDC_ACM_Initialize (void) {
  ...
  return (USBD_CDC_ACM_PortInitialize ());		//串口配置/初始化
}
...
__weak int32_t USBD_CDC_ACM_SetLineCoding (void) 
{
  ...
  return (USBD_CDC_ACM_PortSetLineCoding (&line_coding));			//配置串口
}
...
__weak int32_t USBD_CDC_ACM_GetLineCoding (void) {					//获取串口参数
...
/*******************************************************************************
* 函数名		:	USBD_CDC_ACM_SOF_Event
* 功能描述	: CDC帧首中断处理函数，功能：处理帧首事件，检查否否有数据在等待操作，如果有，则激活读缓冲区操作，并且当有数据要发送时激活发送函数
* 输入			: void
* 返回值		: void
*******************************************************************************/
...
void USBD_CDC_ACM_SOF_Event (void)
{
  ...
  if ((!data_read_access)         &&    /* If not read active                 */
      ...
      (ptr_data_received != USBD_CDC_ACM_ReceiveBuf))	/* and if receive pointer does not already point to the start of the receive buffer */
	{                                                 
	...
	/*输出（电脑）缓冲区还有数据未读取---将数据读取出来*/
  if (data_received_pending_pckts &&    					/* If packets are pending  如果数据包正在等待            */
    ...
    USBD_CDC_ACM_EP_BULKOUT_HandleData(); 				/* Handle received data  处理接收到的数据 */
    data_read_access = 0;               					/* Enable access to read data         */
    if (ptr_data_received != ptr_data_read)
      USBD_CDC_ACM_DataReceived (ptr_data_received - ptr_data_read);  /* Call received callback */
  }
	
	/*还有数据需要上传----激活输入（电脑）处理函数*/	
...
/*******************************************************************************
* 函数名		:	USBD_CDC_ACM_EP_BULKOUT_HandleData
* 功能描述	: 处理接收到的数据
							该函数处理在指传输端点上接收的数据。
							如果有足够的可用空间，它将接收到的数据读取到接收中间缓冲区
* 输入			: void
* 返回值		: void
*******************************************************************************/
...
static void USBD_CDC_ACM_EP_BULKOUT_HandleData ()
{
  ...
  if ((usbd_cdc_acm_receivebuf_sz - (ptr_data_received - USBD_CDC_ACM_ReceiveBuf)) >= usbd_cdc_acm_maxpacketsize1[USBD_HighSpeed])
	{		//((64 - (ptr_data_received - USBD_CDC_ACM_ReceiveBuf)) >= 64)
																					/* If there is space for 1 max packet */
																					/* Read received packet to receive buf*/
    len_received       = USBD_ReadEP(usbd_cdc_acm_ep_bulkout, ptr_data_received);		//读取端点数据存入ptr_data_received并返回已输出（电脑）数据长度
    ptr_data_received += len_received;  	/* Correct pointer to received data   */	//指针地址增加
    if (data_received_pending_pckts &&  	/* If packet was pending              */
       !data_receive_int_access)					/* and not interrupt access           */	//SOF中断时进入未处理完数据
		{
      data_received_pending_pckts--;    	/* Decrement pending packets number   */	//已接收的未处理的数据包个数--减（已处理）
    }
  }
	/*没有存储空间存储*/
	else
	{
    data_no_space_for_receive = 1;      /* There is no space in receive buffer for the newly received data 	*/
    if (data_receive_int_access)				/* If this access is from interrupt	function	*/
		{                                          
      data_received_pending_pckts++;    /* then this is new unhandled packet  */		//已接收的未处理的数据包个数--加（未处理）
    }
  }
}

/*******************************************************************************
* 函数名		:	USBD_CDC_ACM_EP_BULKIN_HandleData
* 功能描述	: 处理端点数据中的大量发送
							该函数处理要在端点上批量发送的数据。
							它发送待发送的数据，这是已经在发送中间缓冲区，
							它也发送零长度的数据包，如果最后发送的数据包不是一个短数据包
* 输入			: void
* 返回值		: void
*******************************************************************************/
...
/*******************************************************************************
* 函数名		:	USBD_CDC_ACM_EP_BULKOUT_Event
* 功能描述	: 串口批量输出处理函数
							1）判断是否正在进行读数操作，如果是，则增加一个待读数操作记录
							2)读取数据
* 输入			: void
* 返回值		: void
*******************************************************************************/
...
void USBD_CDC_ACM_EP_BULKOUT_Event (uint32_t event) 
{
  ...
  if (data_read_access)										/* If data is being accessed from	read function	*/		//正在进行读数操作
	{
    data_received_pending_pckts++;      	/* 1 more packet received and not handled	*/
    return;
  }

  data_read_access = 1;                 	/* Block access to read data          */
  data_receive_int_access = 1;          	/* Read access from interrupt function*/
  USBD_CDC_ACM_EP_BULKOUT_HandleData ();	/* Handle received data               */	//处理接收的数据：根据存储空间是否为满而对数据做接收或者待接收处理
  data_receive_int_access = 0;          	/* Read access from interrupt func end*/
  data_read_access = 0;                 	/* Allow access to read data          */
  if (ptr_data_received != ptr_data_read)	//指针地址不一样
    USBD_CDC_ACM_DataReceived (ptr_data_received - ptr_data_read);    /* Call	received callback	*/
}
...
/*******************************************************************************
* 函数名		:	USBD_CDC_ACM_EP_BULK_Event
* 功能描述	: 虚拟串口端点输入输出函数
* 输入			: void
* 返回值		: void
*******************************************************************************/
...
{
  if (event & USBD_EVT_OUT) 			//电脑输出端点
	{
		USBD_CDC_ACM_EP_BULKOUT_Event (event);		//输出端点，event无效
  }
  if (event & USBD_EVT_IN)				//电脑输入端点
	{
		USBD_CDC_ACM_EP_BULKIN_Event (event);			//输入端点
  }
}
